...
plot "run5_1d3v/disp_E0_x.dat" matrix using (($1-nx/2)*2*pi/dx/nx/normk/n_lineout_raumschritte):($2*2*pi/dt/normfreq/n_timesteps):($3*n_lineout_raumschritte*n_lineout_raumschritte) with image notitle \
...
, 'bernstein' using (kprime($1)/normk):(oprime($2)/normfreq) w l smooth bezier lt 1 lc rgb "#666666" title "Bernstein modes" \

...
class Species:
    ...
    def newv(self, Elocalx):
        ...
        Reads: x ndarray(Np,float): position in cm
               vx ndarray(Np,float): velocity in cm/s
               alpha ndarray(Np,float): dimensionless matrix for B field rotation
...
@njit
def ecsim_current(Nx, x, vx, alpha, q,macro):
    """
...
@njit
def naive_numba_mass_matrix(Nx, x, alpha, q, m, macro, M00):
    """
    ...
    alpha ndarray(Np, float): dimensionless rotation matrices at each
...
def init_fields(B0x):
    ...
    B0x (float): Homogeneous background magnetic field in Gauss

    Returns: ndarray(Nxouter,float) for Bx in Gauss
         and ndarray(Nxouter,float) for Ex in statC/cm
    """
...
def maxwell(Ex, Bx, jhatx, M00, theta):
    """
    ...
    Bx ndarray(Nxouter,float): magnetic field at t^n in Gauss
    ...
    Returns: ndarray(Nxouter,float) for Ex at t^n+1 in statV/cm
             ndarray(Nxouter,float) for Bx at t^n+1 in Gauss
    """
    ...
    timers.tic("build_A")
    A = np.zeros( (2*args.Nxinner,2*args.Nxinner) )
    ...
    newBx[Ng:-Ng] = Fnext[1*args.Nxinner:2*args.Nxinner]
...
def save_energies(energies, t, Bx, Ex, species):
    """
...
B0 = np.sqrt(args.B0x**2)
Wce = e*B0/(me*c)
sys.stderr.write("B0 = "+str(args.B0x)+" G\n")
...
cfg_grp.create_dataset("B0", data=np.array([args.B0x,0.,0.]))
...
energies.flush()
save_energies(energies, 0, Bx, Ex, species)
...
for t in range(1,args.Nt+1):
    ...
    for s in species:
        ...
        Blocalx = s.get_localB(Bx)

        # compute alpha for all particles
        #s.compute_alpha(Blocalx)
        timers.tic("alpha")
        s.alpha = compute_alpha(s.x, Blocalx, s.q,s.m)
        timers.toc("alpha")

        # compute current \hat{j}_sg using x at t^n+1/2 and B and v at t^n
        timers.tic('current')
        jhatx =  ecsim_current(args.Nxouter, s.x, s.vx, s.alpha, s.q,s.macro)
        timers.toc('current')
        ...
        naive_numba_mass_matrix(args.Nxouter, s.x, s.alpha, s.q, s.m, s.macro, M00)
    ...
    newEx,newBx = maxwell(Ex, Bx, jx, M00, args.theta)
    ...
    if args.outputsteps>0 and t%args.outputsteps == 0:
        ...
        saveh5(Bx, "/Timestep_"+str(t)+"/felder/B/B[0]")
    ...
    for s in species:
        ...
        jxsgbar = ecsim_current(args.Nxouter, s.x, s.vxbar, s.alpha, s.q,s.macro)
        jxgbar += jxsgbar
    ...
    deltaEfields = deltaEfieldsEx + deltaEfieldsBx
    ...
    save_energies(energies, t, Bx, Ex, species)
